...
const Card16Podcast: FC<Card16PodcastProps> = ({
  ...
  return (
    ...
    >
      ...
      >
        <>
        <a href={href} className={`block flex-shrink-0 relative w-full rounded-3xl overflow-hidden ${ratio}`}>
      ...
      <div className="w-11/12 transform -mt-32 ">
        ...
        <div className="p-5 mt-5   dark:bg-neutral-900 shadow-xl dark:shadow-2xl rounded-3xl rounded-tl-none flex flex-col flex-grow ">
          <h2 className="nc-card-title block text-xl font-semibold text-neutral-900 dark:text-neutral-100 ">
          {/* className="line-clamp-1" title={title} */}
            <Link href={href} >
            <a href={href} className="line-clamp-1">{title}</a>
            </Link>
          </h2>
