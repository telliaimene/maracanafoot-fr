...
const Badge: FC<BadgeProps> = ({
  ...
  return !!href ? (
    ...
    <Link
      href={href || ""}
      style={{backgroundColor: "#061da5",color: "#fff",fontWeight: "bold"}}
    >
          <a href=""   style={{backgroundColor: "#061da5",color: "#fff",fontWeight: "bold"}}  className={`transition-colors hover:text-white duration-300 ${CLASSES} `}>{name}</a>
     
    </Link>
  ) : (
    <span  style={{backgroundColor: "#061da5",color: "#fff",fontWeight: "bold"}}  className={`${CLASSES} ${getColorClass(false)} ${className}`}>
      {name}
    </span>
  );
};
