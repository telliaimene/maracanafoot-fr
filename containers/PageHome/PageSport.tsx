...
const SportsPage = () => {
  ...
  //  { <Spinner></Spinner
  useEffect(() => {
    setTournoi(router? router.query ? router.query.id ? router.query.id[1] : undefined : undefined : undefined)
  ...
  async function getFlashInfos() {
    ...
    await axios.get(API_LINK + URL)
      .then((response) => {
        ...
        let table: ArticleDataType[] = response.data.map((item: ArticleType) => {
          return ({
...
console.log("all article", internationnal2)
 ...
 <PostCarousel data={allArticle} /> 
      ...
      {topComment.length || topCount.length || topShare.length ? <PostGallery topComment={topComment} topCount={topCount} topShare={topShare} /> : null}
      {fennec.length ? <FeatureNews data={fennec} /> : null}
      <div className="container">
        <div className="row">
          <div className="col-lg-8">
           {internationnal2.length?<TrendingNews data={internationnal2} data2={internationnal2} />:null} 
          </div>
      ...
      <div className="entertrainments">
        ...
        {   internationnal4.length ?<BusinessNews  data={internationnal4} /> : null}
            </div>
            <div className="col-lg-4">
              <div className="row">
